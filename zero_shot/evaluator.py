...
def simple_evaluate(
    ...
    if args.method != 'full':
        dataloader, testloader = get_loaders(
            args.calib_data, nsamples=args.nsamples, seed=args.seed, model=args.model, seqlen=lm.seqlen
        )
        tick = time.time()
        if 'opt' in args.model:
            quantizers = lm.opt_sequential(dataloader)
        else:
            quantizers = lm.llama_sequential(dataloader)
        print('sequential time (min): ', (time.time() - tick)/60)
...
def evaluate(
    ...
    for reqtype, reqs in requests.items():
        ...
        for resp, (i, task_name, doc, doc_id) in zip(resps, requests_origin[reqtype]):
            # print(f'process_res_queue: resp={resp}, i={i}, task_name={task_name}, doc_id={doc_id}')
            process_res_queue[(task_name, doc_id)].append((i, resp))
        print('process_res_queue time (seconds): ', time.time() - tick)
    ...
    for (task_name, doc_id), requests in process_res_queue.items():
        ...
        # print(f'process_results: task_name={task_name}, doc_id={doc_id}')
